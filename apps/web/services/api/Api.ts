...
class Api {
  ...
  static async auth(username: string, password: string) {
    ...
    return this.processAuthResponse(res);
  }
  ...
  /** Method to login user from Api response
   * Add the access token and user data to local storage
   * Save refresh token in the cookie (credentials: include)
   ...
   * @return user
   */
  private static async processAuthResponse(res: Response) {
    if (!res.ok) {
